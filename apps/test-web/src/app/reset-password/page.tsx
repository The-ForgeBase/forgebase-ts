...
export default function ResetPasswordPage() {
  ...
  useEffect(() => {
    const verifyToken = async () => {
      try {
        ...
        setTokenValid(false);
        setStatus('error');
        setMessage(error?.message || 'Failed to verify reset token. Please request a new password reset link.');
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setStatus('error');
      setMessage(error?.message || 'Failed to reset your password. Please try again or contact support.');
    }
  };
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-100">
      <div className="bg-white p-8 rounded-lg shadow-md w-full max-w-md">
        ...
        {status === 'submitting' && (
          <div className="flex flex-col items-center justify-center">
            <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-500 mb-4"></div>
            <p className="text-gray-600 text-center">Resetting your password...</p>
          </div>
        )}
