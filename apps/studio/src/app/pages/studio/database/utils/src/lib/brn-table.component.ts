...
@Component({
  ...
  providers: [
    provideTableClassesSettableExisting(<T>() => BrnTableComponent<T>),
  ],
  template: `
    <cdk-table
      #cdkTable
...
export class BrnTableComponent<T>
  implements TableClassesSettable, AfterContentInit
{
  ...
  @ContentChildren(BrnColumnDefComponent)
  public columnDefComponents!: QueryList<BrnColumnDefComponent>;
  ...
  public ngAfterContentInit(): void {
    ...
    this.columnDefComponents.forEach((component) => {
      if (component.cell) {
        // To avoid duplicate column definitions, first check if it's already added
        try {
          this._cdkTable!.addColumnDef(component.columnDef);
        } catch (e) {
          console.warn('Column might already be registered:', component.name);
        }
      }
    });

    // Add all row definitions
