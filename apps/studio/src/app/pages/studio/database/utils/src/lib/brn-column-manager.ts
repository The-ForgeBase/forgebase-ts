...
export class BrnColumnManager<T extends BrnColumnVisibility> {
  private _initialColumnVisibility: T;
  private _columnVisibility = signal<T>({} as T);

  public allColumns: AllColumnsPropertyType<T>;
  public columnVisibility;
  public displayedColumns: Signal<(keyof T)[]> = computed(() => {
    // console.log('displayedColumns', this._columnVisibility());
  ...
  public resetVisibility(initialColumnVisibility: T) {
    this._initialColumnVisibility = initialColumnVisibility;
    this._columnVisibility.set(this._initialColumnVisibility);
    this.columnVisibility = this._columnVisibility.asReadonly();
    this.allColumns = this.createAllColumns(this._columnVisibility());
    // console.log(this._columnVisibility());
  }
  ...
  public setVisible(columnName: keyof T) {
    ...
    const columnEntry = visibilityMap[columnName];
    const newVisibilityState =
      typeof columnEntry === 'boolean' ? true : { visible: true };
    this._columnVisibility.set({
      ...visibilityMap,
      [columnName]: newVisibilityState,
    });
  ...
  public setInvisible(columnName: keyof T) {
    ...
    const columnEntry = visibilityMap[columnName];
    const newVisibilityState =
      typeof columnEntry === 'boolean' ? false : { visible: false };
    this._columnVisibility.set({
      ...visibilityMap,
      [columnName]: newVisibilityState,
    });
  ...
  private createAllColumns(
    initialColumnVisibility: T
  ): AllColumnsPropertyType<T> {
...
export const useBrnColumnManager = <T extends BrnColumnVisibility>(
  initialColumnVisibility: T
