...
import { AuthInput, ValidationResult } from '../types';
...
export function validateWithStandardSchema<T extends StandardSchemaV1>(
  ...
  try {
    ...
    return {
      isValid: false,
      errors: {
        [fieldName]: error instanceof Error ? error.message : 'Unknown validation error'
      }
    };
  }
}
 ...
 * @returns ValidationResult compatible with reauth's validation system
 */
export async function validateInputWithStandardSchema<T extends StandardSchemaV1>(
  schema: T,
  ...
  try {
    ...
    return {
      isValid: false,
      errors: {
        _error: error instanceof Error ? error.message : 'Unknown validation error'
      }
    };
  }
}
