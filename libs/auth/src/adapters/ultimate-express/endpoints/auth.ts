...
import { setAuthCookies, extractTokenFromRequest, extractRefreshTokenFromRequest } from '../utils';
...
export function createAuthRouter(authManager: DynamicAuthManager,  adminManager: InternalAdminManager, config: ExpressAuthConfig): Router {
  const router = Router();


  // Registration endpoint
  router.post('/register', async (req: Request, res: Response): Promise<any | Response> => {
    try {
      const { provider, password, ...credentials } = req.body;
      const result = await authManager.register(provider, credentials, password);
  ...
  // Login endpoint
  router.post('/login', async (req: Request, res: Response): Promise<any | Response> => {
    try {
      ...
      if (!result.user && provider === 'passwordless') {
        return res.status(200).json({
          success: true,
          message: 'Passwordless login initiated, check your email or sms or whatsapp for verification code',
          exp: '15m',
        });
      }
  ...
  // Logout endpoint
  router.get('/logout', async (req: Request, res: Response): Promise<any | Response> => {
    try {
  ...
  router.post('/refresh-token', authGuard, async (req: Request, res: Response): Promise<any | Response> => {
    try {
  ...
  router.get('/me', authGuard, async (req: ExpressAuthRequest, res: Response): Promise<any | Response> => {
    try {
