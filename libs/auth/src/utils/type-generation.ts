...
export function generateTypeInterface(
  ...
  const fieldDefinitions = fields
    .map((field) => {
      ...
      const comment = includeComments && field.description 
        ? `  /** ${field.description} */\n` 
        : '';
...
export function generateTypeFile(
  ...
  const interfaceCodeWithoutImport = interfaceCode
    .replace(`import { ${baseInterface} } from '${importPath}';`, '');
...
export function generateExtensionTypes(
  ...
  const imports = [`import { ${baseOptions.baseInterface || 'User'} } from '${baseOptions.importPath || '@forgebase-ts/auth'}';`];
...
export function generateCombinedUserType(
  ...
  const imports = [
    `import { ${baseInterface} } from '${importPath}';`,
    ...extensionTypes.map(type => `import { ${type} } from './${type.toLowerCase()}';`),
  ];
