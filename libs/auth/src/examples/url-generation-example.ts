...
export async function setupWithUrlGeneration(knex: Knex) {
  ...
  const verificationUrl = verificationService.generateVerificationUrl(verificationToken, user.id);
  ...
  const generatedToken = await verificationService.sendVerificationEmail(user.email, user);
  ...
  const generatedResetToken = await verificationService.sendPasswordResetEmail(user.email, user);
