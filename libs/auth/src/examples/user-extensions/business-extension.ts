 ...
 */
export async function extendUserTableWithBusinessFields(knex: Knex): Promise<void> {
...
export async function businessExtensionExample(knex: Knex): Promise<void> {
  ...
  console.log(`${updatedUser.name} works at ${updatedUser.company_name} as ${updatedUser.job_title}`);
  console.log(`Business address: ${businessAddress.street}, ${businessAddress.city}, ${businessAddress.state} ${businessAddress.zip}`);
}
